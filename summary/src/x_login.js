...
// Construct absolute path to .dev.vars relative to this script's directory
const envPath = path.join(__dirname, '..', '.dev.vars');
console.log(`Attempting to load environment variables from: ${envPath}`); // Log path for debugging
require('dotenv').config({ path: envPath });

const { Hono } = require('hono');
const { xAuth } = require('@hono/oauth-providers/x');
const { serve } = require('@hono/node-server'); // Use for Node.js; remove for Cloudflare Workers or Deno
...
if (!process.env.client_id || !process.env.client_secret) {
	console.error('Error: client_id or client_secret not found in .dev.vars or environment.');
...
const twitterAuthMiddleware = xAuth({
	...
	redirect_uri: 'http://localhost:8787/x/callback', // Verify this matches Developer Portal
});
...
app.get('/x/callback', twitterAuthMiddleware, async (c) => {
	...
	if (token) {
		...
		return c.text('已成功獲取 Twitter 存取權杖！請查看您的伺服器控制台。您可以關閉此視窗。');
	} else {
		...
		return c.text(`獲取 Twitter 存取權杖失敗。錯誤: ${error || '未知'}，描述: ${errorDescription || '無'}。請查看伺服器控制台。`, 500);
	}
});
...
serve(
	...
	() => {
		console.log(`伺服器已啟動，監聽端口 ${port}。請訪問 http://localhost:${port}/x/login`);
	}
);
